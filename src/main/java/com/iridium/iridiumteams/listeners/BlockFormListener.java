...
public class BlockFormListener<T extends Team, U extends IridiumUser<T>> implements Listener {
    ...
    public void onBlockForm(BlockFormEvent event) {
        iridiumTeams.getTeamManager().getTeamViaLocation(event.getBlock().getLocation()).ifPresent(team -> {
            TeamSetting teamType = iridiumTeams.getTeamManager().getTeamSetting(team, SettingType.ICE_FORM.getSettingKey());
            if (teamType.getValue().equalsIgnoreCase("Disabled") && (event.getNewState().getType() == Material.ICE || event.getNewState().getType() == Material.SNOW)) {
